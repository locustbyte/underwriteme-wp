...
Plugin URI: http://www.mehmetcelik.com/fp-demo
Author: Mehmet Celik
Author URI: http://www.mehmetcelik.com/fp-demo
Version: 1.6.0
Description: Create beautiful scrolling fullscreen web sites with Visual Composer, fast and simple. Visual Composer Addon of FullPage JS implementation.
...
if (!class_exists('MCW_FullPage'))
{
  ...
  {
    // FullPageJS version
    protected $fullpage_js_version = '2.8.5';
    ...
    private function getAsset($folder, $params = array())
    {
      ...
      $deneme['before'] = array();
      $deneme['before']['file'] = 'vc_anim.js';
      $deneme['afterLoad'] = 'mcw_vcanim_al(this);';
      $deneme['afterSlideLoad'] = 'mcw_vcanim_asl(this);';
              ...
              if (isset($value) && !empty($value))
              {
                ...
                if (is_array($value))
                {
                  if (isset($value['file']))
                  {
                    ...
                    {
                      $events[$key] = file_get_contents($content_file);
                    }
                    // Update content parameters
                    if (isset($value['params']))
                    {
                      foreach ($value['params'] as $param => $value)
                      {
                        ...
                        $events[$key] = str_replace('[@'.$param.']', $value, $events[$key]);
                      }
                    }
                  }
                }
    ...
    public function on_wp_enqueue_scripts()
    {
      ...
      if ( $this->is_field_enabled($this->id_fullPageEnable) )
      {
        ...
        $isFooter = $this->is_field_enabled($this->id_fullPageEnable);
        $dep = array('jquery');
    ...
    public function on_wp_head()
    {
      ...
      if ( $this->is_field_enabled($this->id_fullPageEnable) )
      {
          ...
          if ( isset( $found[3] ) && is_array( $found[3] ) )
          {
            foreach ( $found[3] as $key => $shortcode_atts )
            {
              ...
              if (isset($atts[$this->vc_SectionBehaviour]) && $atts[$this->vc_SectionBehaviour] == 'fixed_top')
              {
                ...
                // Add fixed top css
                $css .= '.'.$this->cn_fixed_top.'{position: fixed; z-index: 999999; top: 0px; width: 100%;}';
                continue;
              }
              ...
              if (isset($atts[$this->vc_SectionBehaviour]) && $atts[$this->vc_SectionBehaviour] == 'fixed_bottom')
              {
                ...
                $css .= '.'.$this->cn_fixed_bottom.'{position: fixed; z-index: 999999; bottom: 0px; width: 100%;}';
                continue;
              }
        ...
        // Design parameters
        {
          ...
          // paddingTop
          $parameters[] = 'paddingTop:"'.$this->get_field_value($this->id_PaddingTop, '0px').'"';
          // paddingBottom
        ...
        if (!empty($tooltip))
        {
          $tooltipCSS .= 'background-color: '.$tooltip.';';
        }
        // Tooltip text color
        ...
        if (!empty($tooltipCSS))
        {
          $css .= '#fp-nav ul li .fp-tooltip{padding:0 5px;'.$tooltipCSS.'}';
        }
    ...
    protected function init_meta_box()
    {
      ...
      $nav_accordion_fields = array(
        ...
        'fields' => array(
          ...
          array(
            ...
            'value' => 'off',
            'description' => __('This parameter determines the position of navigation bar which is made up of small circles.', $this->tag),
          ),
          // showActiveTooltip
          array(
          ...
          array(
            ...
            'value' => 'off',
            'description' => __('This parameter determines the position of landscape navigation bar for sliders which is made up of small circles.', $this->tag),
          ),
          // controlArrows
          array(
      ...
      $groups = array($full_page_enable_section, $nav_accordion_fields, $scrolling_accordion_fields,
        $design_accordion_fields, $customizations_accordion_fields, $events_accordion_fields,
    ...
    public function on_admin_init()
    {
      ...
      if(function_exists('vc_add_param'))
      {
        ...
        vc_add_param( 'vc_row', array(
          ...
          'value' => '',
          'description' => __('Enter an anchor name if you want to use anchors.', $this->tag),
          'dependency' => array('element' => $this->vc_SectionBehaviour, 'value' => array('off', 'on')),
          'group' => $this->vcGroupName,
        ) );
