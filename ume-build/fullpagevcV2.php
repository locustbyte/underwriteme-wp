...
(function() {
  ...
  var settings = {
    NUM_PARTICLES : 300,
    DISTANCE_T    : Math.round(dcurrentValue),
    RADIUS        : Math.round(currentValue),
    OPACITY       : 0.5,
    SPEED_X       : 0.1,
    SPEED_Y       : 0.1,
    AMPLITUDE     : 0
  ...
  var COLOURS    = ["#2d373e", "#2d373e", "#E17122"],
      bounds     = {},
      particles  = [],
      random     = Math.random,
      sqrt       = Math.sqrt,
  ...
  var counter = setInterval(timer, quadratic(1500000,8,1)); 
  ...
  var dcount = 1;
  var dcounterIncrement = 1;
  var dcounter = setInterval(ttimer, 1000); 

  function ttimer() {
    dcount = dcount+dcounterIncrement;
    if (dcount == 1 || dcount == 16000000 ) {
          dcounterIncrement = -dcounterIncrement;
      }
    //console.log(dcount);
    updateDistance(dcount)
  }
  ...
  var updateRadius = function( current ) {
    for (var i = 0; i < settings.NUM_PARTICLES; i += 1) {
      particles[i].radius = current;
      
    }
  };

  var updateDistance = function( dcurrent ) {
    for (var i = 0; i < settings.DISTANCE_T; i += 1) {
      particles[i].Distance = dcurrent;
      particles[i].radius = dcurrent;
